...
from jeffrey.config_schemas.tokenizer_training_config_schema import TokenizerTrainingConfig
from jeffrey.utils.io_utils import write_yaml_file
from jeffrey.utils.utils import get_logger
from jeffrey.utils.config_utils import get_pickle_config


@get_pickle_config(config_path="jeffrey/configs/automatically_generated", config_name="tokenizer_training_config")
def train_tokenizer(config: TokenizerTrainingConfig) -> None:
    ...
    tokenizer.train(texts=df[text_column_name].values)
    
    logger.info("Saving tokenizer...")
    tokenizer_save_dir = os.path.join(os.path.dirname(data_parquet_path), "trainer_tokenizer")
    tokenizer.save(tokenizer_save_dir=tokenizer_save_dir)
