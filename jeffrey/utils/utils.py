...
class SpellCorrectionModel:
    def __init__(self, max_dictionary_edit_distance: int = 2, prefix_length: int = 7, count_threshold: int = 1) -> None:
        self.dictionary_path = pkg_resources.resource_filename(
            package_or_requirement="symspellpy", resource_name="frequency_dictionary_en_82_765.txt"
        )
        self.bigram_dictionary_path = pkg_resources.resource_filename(
            package_or_requirement="symspellpy", resource_name="frequency_bigramdictionary_en_243_342.txt"
        )
    ...
    def _initialize_model(self) -> SymSpell:
        ...
        model.load_dictionary(corpus=self.dictionary_path, term_index=0, count_index=1)
        model.load_bigram_dictionary(corpus=self.bigram_dictionary_path, term_index=0, count_index=2)

        return model

    def __call__(self, text: str) -> str:
        suggestion: str = self.model.lookup_compound(phrase=text, max_edit_distance=self.max_dictionary_edit_distance)[
            0
        ].term
        return suggestion
