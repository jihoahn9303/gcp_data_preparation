...
class DatasetCleaner(ABC):
    def __call__(self, text: Union[str, List[str]]) -> Union[str, List[str]]:
        ...
        return self.clean_words(words=text)
...
class URLDatasetCleaner(DatasetCleaner):
    def clean_text(self, text: str) -> str:
        return re.sub(pattern=r"http\S+", repl="", string=text, flags=re.MULTILINE)
...
class SpellCorrectionDatasetCleaner(DatasetCleaner):
    ...
    def clean_text(self, text: str) -> str:
        cleaned_text: str = self.spell_correction_model(text)
        return cleaned_text
...
class DatasetCleanerManager:
    ...
    def __call__(self, text: Union[str, List[str]]) -> Union[str, List[str]]:
        for dataset_cleaner_name, dataset_cleaner in self.dataset_cleaners.items():
            if isinstance(text, str):
                text = dataset_cleaner.clean_text(text)
            elif isinstance(text, list):
                text = dataset_cleaner.clean_words(text)
            else:
